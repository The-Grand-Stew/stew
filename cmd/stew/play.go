...
// 	var answer string
// 	logging.ShowMessage("command", "Run the command below: "+commandStr, true, true)
...
// 		logging.ShowMessage("command", "\n", true, false)
// 		return true
// 	} else {
// 		logging.ShowError("That doesn't seem like the right command! Try again")
// 	}
// 	logging.ShowError("That doesn't seem like the right command!")
// 	return false
// }
...
// 	logging.ShowMessage("doc", introContent, true, true)
// 	var answer string
...
// 		clear.Run()
// 	} else {
// 		logging.ShowMessage("doc", "Maybe not today! See you next time", true, true)
// 		os.Exit(0)
// 	}
...
// 	logging.ShowMessage("doc", `First, create a new Project called "microservices-demo"`, true, false)
// 	logging.ShowMessage("doc", `This will also prompt you to create your first service. Let's call it "products" which runs on port "8000". Lets make this service in Go`, true, false)
// 	if run = generateCommand("stew init"); run {
// 		createProject()
// 	}
// 	logging.ShowMessage("doc", `Great! Let's create another microservice"`, true, true)
// 	logging.ShowMessage("doc", `First, Go to your project folder`, true, true)
// 	if run = generateCommand("cd " + Config.ProjectName); run {
// 		os.Chdir(Config.ProjectName)
// 	}
// 	logging.ShowMessage("doc", `Create another service called "payments" that runs on port 8001 and choose node as your preferred language`, true, false)
// 	if run = generateCommand("stew create-service"); run {
// 		createService()
// 	}
// 	logging.ShowMessage("success", `There! You now have 2 services configured! You can take a look at your project directory to see if you have the boilerplate code generated. (This will open a window in VS code. Make sure you have the command line tool for VS Code installed. Else just open your project in your favorite editor)`, true, true)
...
// 	}
// 	logging.ShowMessage("doc", `Now consider the "products" service uses Postgres database. Lets add some code for connecting to the database. `, true, false)
// 	if run = generateCommand("stew add-database"); run {
// 		createDatabase()
// 		os.Chdir(currentDir)
// 	}
// 	logging.ShowMessage("doc", `Next, We realise that the "payments" service needs an additional domain called "internationalPayments". Let's add that in... `, true, false)
...
// 	logging.ShowMessage("success", `Now that we have some code in place, let's try deploying to our favorite cloud`, true, false)
...
// 		os.Exit(0)
// 	}
// 	logging.ShowMessage("doc", `The first step is to create a base infrastructure for your microservices. Mostly things like a VPC, API gateways. It depends on the flavor of deployment you choose.`, true, false)
// 	logging.ShowMessage("doc", `For this example, we are going to deploy to "AWS" using "ECS Fargate" containers. (This might take a while..but hold on!)`, true, false)
// 	logging.ShowMessage("doc", `Select AWS as your cloud provider with "eu-west-1" as your region and "dev" as your environment when prompted.`, true, false)
...
// 	logging.ShowMessage("sucess", `Good! We have our base setup in place. Time to deploy our services.`, true, false)

// 	logging.ShowMessage("docs", `Let's deploy the "products" service for now. The same process can be repeated for the other services too"`, true, false)
// 	if run = generateCommand("stew deploy"); run {
// 		os.Chdir(currentDir)
// 		createDeployment()
// 	}
// 	logging.ShowMessage("success", "That's All Folks! It was easy, right? You can make your own customisations, write your own business logic or create more services! Its your arena now!", true, true)
// }
